#include "../../YTPublic/YTInclude/YTUnit/YTBlowfish.h"


CYTBlowfish::CYTBlowfish(void)
{
...
CYTBlowfish::CYTBlowfish(BYTE *pKey, short keybytes)
{
	// copy the defaults into our member boxes
	for(int i=0; i<bf_N+2; i++)
	{
...
void CYTBlowfish::InitializeBlowfish(BYTE *pKey, short keybytes)
{
	union aword temp;
...
BOOL CYTBlowfish::Encrypt(BYTE *pData, DWORD dwDataLen)
{
...
BOOL CYTBlowfish::Decrypt(BYTE *pData, DWORD dwDataLen)
{
	if(((dwDataLen/8)*8) != dwDataLen)
	{
		return FALSE;
...
void CYTBlowfish::Blowfish_encipher(DWORD *xl, DWORD *xr)
{
	union aword  Xl;
	union aword  Xr;
...
void CYTBlowfish::Blowfish_decipher(DWORD *xl, DWORD *xr)
{
	union aword  Xl;
	union aword  Xr;
